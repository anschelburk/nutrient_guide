...
def button_add_to_list(
    ...
    list_add_button = st.button('Add Ingredient to My List')
    ...
    if list_add_button:
        ...
        if not api_ingredient_name in cached_ingredients_dict.keys():
            ...
            # Add to database
            for nutrient_name, nutrient_data in api_ingredient_nutrients.items():
                database_functions.add_ingredient(
                    db,
                    api_ingredient_name,
                    nutrient_name,
                    nutrient_data['value'],
...
def button_remove_from_list(
    ...
    if st.session_state[f'remove_button_{api_ingredient_name}']:
        ...
        cached_ingredient_quantity = cached_ingredients_dict[api_ingredient_name]['quantity']
        ...
        if api_ingredient_name in cached_ingredients_dict.keys():
            ...
            database_functions.delete_ingredient(db, api_ingredient_name)
...
def modify_dicts(current_nutrients: dict,
    ...
    current_nutrients, new_nutrients = _normalize_nutrient_names(current_nutrients, new_nutrients)
...
def update_ingredient_quantities(
    ...
    with ingredients_list:
        ...
        list_of_my_nutrients = list(cached_ingredients.keys())
        ...
        for item in list_of_my_nutrients:
            ...
            if dropdown_option != cached_ingredient_quantity:
                ...
                if dropdown_option == 'Remove ingredient from list':
                    database_functions.delete_ingredient(db, item)
                
                else:

                    for nutrient_name, nutrient_data in cached_ingredient_nutrients.items():
                        db_ingredient = database_functions.get_ingredient_by_name(db, item)

                        if db_ingredient:
                            db_ingredient.nutrient_value = nutrient_data['value'] * dropdown_ingredient_quantity
                            db.commit()
...
if __name__ == '__main__':
    ...
    nutrient_guide_database = next(database_functions.get_db())
